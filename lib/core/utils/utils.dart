...
class PDFGenerator {
  static Future<void> generatePDF(
    ...
    final ByteData bytes = await rootBundle.load(AppAssets.lienzo);
              ...
              pw.Padding(
                ...
                child: pw.Column(
                  ...
                  children: <pw.Widget>[
                    ...
                    pw.SizedBox(height: 20),
                    pw.Text('Empresa: ${item['empresa'] ?? ''}',
                        style: const pw.TextStyle(fontSize: 14)),
                    pw.SizedBox(height: 10),
                    pw.Text('Descripción: ${item['descripcion'] ?? ''}',
                        style: const pw.TextStyle(fontSize: 14)),
                    pw.SizedBox(height: 10),
                    pw.Text('Nota: ${item['nota'] ?? ''}',
                        style: const pw.TextStyle(fontSize: 14)),
                    pw.SizedBox(height: 20),
                    pw.Divider(),
                  ],
    ...
    try {
      ...
      final file =
          File('${directory.path}/${item['nombreProyecto'] ?? 'proyecto'}.pdf');
      final pdfBytes = await pdf.save();
      await file.writeAsBytes(pdfBytes);

      //context.push('/${PdfPreviewView.name}/${file.path}');

      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => PdfPreviewView(filePath: file.path),
        ),
      );
    } catch (e) {
